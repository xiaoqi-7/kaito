...
We measured guardrail processing overhead using a deterministic microbenchmark that isolates scanner execution from model inference and network variance. The microbenchmark ran on an AMD Ryzen 9 8945HS (8 cores, 16 threads), 24 GB RAM, WSL2 Linux 5.15, Python 3.12. Figure 1 plots the P50 overhead against output token count for three configurations. All three scale linearly with response length. The [benchmark code, test cases, and reproduction instructions](https://github.com/kaito-project/kaito/tree/main/benchmarks/ragengine_guardrails#reproducing-figure-1) are available in the KAITO repository.

![Figure 1. Guardrail processing overhead vs output tokens (P50, deterministic microbenchmark, chunk_size=20, 100 iterations).](../benchmarks/ragengine_guardrails/results/overhead_vs_tokens.png)

Table 4 summarizes the overhead rate (slope) for each configuration. Applications can estimate guardrail processing cost for any response length by multiplying the rate by the expected output token count.
...
In live runs against Phi-4-mini (3.67 s baseline) and mistral-small (2.84 s baseline), enabling guardrails produced no observable change in median end-to-end latency, consistent with sub-millisecond to millisecond-scale processing overhead relative to multi-second model inference.
